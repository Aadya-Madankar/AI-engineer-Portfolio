...
function T(i) {
    getComputedStyle(i).position === "sticky" && (i.style.setProperty("position", "static"),
    i.dataset.sticky = "true"),
    i.offsetParent && T(i.offsetParent)
}
function v(i) {
    i?.dataset?.sticky === "true" && (i.style.removeProperty("position"),
    delete i.dataset.sticky),
...
var H = class {
    ...
    setRect({top: i, left: e, width: t, height: s, element: o}={}) {
        ...
        !(i === this.rect.top && e === this.rect.left && t === this.rect.width && s === this.rect.height && o === this.rect.element) && (this.rect.top = i,
        this.rect.y = i,
        this.rect.width = t,
        this.rect.height = s,
        this.rect.left = e,
    ...
    onWrapperResize = () => {
        let i, e;
        if (this.options.ignoreSticky && T(this.element),
        this.options.ignoreTransform)
            i = P(this.element),
            e = k(this.element);
        else {
            const t = this.element.getBoundingClientRect();
            i = t.top + x(this.element),
            e = t.left + z(this.element)
        }
        this.options.ignoreSticky && v(this.element),
        this.setRect({
            top: i,
            left: e
        })
...
var q = class {
    constructor(i, {type: e="proximity", lerp: t, easing: s, duration: o, distanceThreshold: n="50%", debounce: h=500, onSnapStart: c, onSnapComplete: S}={}) {
        this.lenis = i,
        this.options = {
            type: e,
            lerp: t,
    ...
    addElement(i, e={}) {
        const t = w();
        return this.elements.set(t, new H(i,e)),
    ...
    onSnap = () => {
        ...
        if (this.elements.forEach( ({rect: r, align: g}) => {
            let a;
            g.forEach(d => {
                d === "start" ? a = r.top : d === "center" ? a = e ? r.left + r.width / 2 - this.viewport.width / 2 : r.top + r.height / 2 - this.viewport.height / 2 : d === "end" && (a = e ? r.left + r.width - this.viewport.width : r.top + r.height - this.viewport.height),
                typeof a == "number" && t.push({
                    value: Math.ceil(a),
                    userData: {}
                })
            }
        ...
        typeof this.options.distanceThreshold == "string" && this.options.distanceThreshold.endsWith("%") ? f = Number(this.options.distanceThreshold.replace("%", "")) / 100 * this.viewport[p] : typeof this.options.distanceThreshold == "number" ? f = this.options.distanceThreshold : f = this.viewport[p],
        (this.options.type === "mandatory" || this.options.type === "proximity" && S <= f) && this.lenis.scrollTo(c.value, {
...
class E {
    constructor(e, t) {
        ...
        this.init(),
        this.loading(),
        window.matchMedia("(min-width: 768px)").matches && (this.snaps = new b(this.main,this.app,this.calculateSnaps())),
        this.app.on("resize", () => this.resize()),
        this.app.on("destroy", () => this.destroy())
    }
    calculateSnaps() {
        return this.heroHeight = this.sections[0].offsetHeight - window.innerHeight,
        ...
        this.heroTop = 0,
        this.heroSnaps = [window.innerHeight * .8, this.heroHeight * .41, this.heroHeight * .6, this.heroHeight * .8],
        this.secondSectionTop = this.getOffsetTop(this.sections[1]),
        this.secondSectionSnaps = [this.secondSectionTop + this.secondSectionHeight * .4, this.secondSectionTop + this.secondSectionHeight * .55, this.secondSectionTop + this.secondSectionHeight * .67, this.secondSectionTop + this.secondSectionHeight * .79],
        this.thirdSectionTop = this.getOffsetTop(this.sections[2]),
        this.thirdSectionSnaps = [this.thirdSectionTop + this.thirdSectionHeight * .4, this.thirdSectionTop + this.thirdSectionHeight * .55, this.thirdSectionTop + this.thirdSectionHeight * .67, this.thirdSectionTop + this.thirdSectionHeight * .79],
        this.footerTop = this.getOffsetTop(this.main.querySelector("footer")),
        [this.heroTop, ...this.heroSnaps, this.secondSectionTop + window.innerHeight * .1, ...this.secondSectionSnaps, this.thirdSectionTop, ...this.thirdSectionSnaps, this.footerTop]
    }
    init() {
        this.hero = {},
        this.hero.titleSplit = new u(this.sections[0].querySelector("h1"),{
            type: "chars, lines"
        }),
        this.hero.titleSplitSecond = new u(this.sections[0].querySelector("h1"),{
            type: "chars"
        }),
    ...
    createHeroAnimation() {
        ...
        this.hero.tl = l.timeline({
            defaults: {
                ease: "power2",
                duration: 1
            }
        }),
        this.hero.tl.to(this.hero.titleSplitSecond.chars, {
            yPercent: -110,
            stagger: .02
        }, 0).fromTo(this.sections[0].querySelector("h2"), {
            autoAlpha: 0
        }, {
            autoAlpha: 1
        }, "-=0.2").fromTo(this.hero.secondTitle.chars, {
            yPercent: 110
        }, {
            yPercent: 0,
            stagger: .02
        ...
        const e = 480;
        l.matchMedia().add({
            isLarge: `(min-width: ${e}px)`,
            isSmall: `(max-width: ${e - 1}px)`
        }, t => {
            const {isLarge: s, isSmall: o} = t.conditions;
            s ? (this.hero.colsSplites.forEach( (n, h) => {
                const c = h === 0 ? "-=0.4" : "<60%";
                this.hero.tl.fromTo(n.split.lines, {
                    yPercent: 110
                }, {
                    yPercent: 0,
                    stagger: .05
            ...
            this.hero.tl.fromTo(this.sections[0].querySelectorAll(".about_text"), {
                yPercent: 110
            }, {
                yPercent: 0,
                stagger: .05
            }, "<60%")) : o && this.hero.colsSplites.forEach( (n, h) => {
                const c = h === 0 ? "-=0.4" : ">";
                this.hero.tl.fromTo(n.split.lines, {
                    yPercent: 110
                }, {
                ...
                h !== this.hero.colsSplites.length - 1 ? this.hero.tl.to(n.split.lines, {
                    yPercent: -110,
                    stagger: .05
                }, ">0.4") : this.hero.tl.fromTo(this.sections[0].querySelectorAll(".about_text"), {
                    yPercent: 110
                }, {
                    yPercent: 0,
                    stagger: .05
    ...
    createBgParallax(e, t=!0) {
        let s, o;
        const n = window.innerHeight * .5;
        t && (s = l.timeline({
            paused: !0
        }),
        s.fromTo(e.querySelectorAll("img"), {
            y: -n
        }, {
            y: 0,
            ease: "none",
        ...
        h.fromTo(e.querySelectorAll("img"), {
            y: 0
        }, {
            y: n,
            immediateRender: !1,
    ...
    createBgAnimation(e, t="top bottom", s="bottom bottom-=100%", o=!0) {
        ...
        const n = l.timeline({});
        n.fromTo(e.querySelectorAll("img"), {
            scale: 1.001
        }, {
            scale: 1.09
        });
    ...
    createSectionAnimation(e) {
        ...
        const h = 480;
        l.matchMedia().add({
            isLarge: `(min-width: ${h}px)`,
            isSmall: `(max-width: ${h - 1}px)`
        }, p => {
            const {isLarge: r, isSmall: g} = p.conditions;
            r ? o.forEach( (a, d) => {
                const y = d === 0 ? ">0.2" : ">";
                n.fromTo([a.title, a.split.lines], {
                    yPercent: 110
                }, {
                    yPercent: 0,
                    stagger: .05
                ...
                d !== o.length - 1 && n.to([a.title, a.split.lines], {
                    yPercent: -110,
                    stagger: .05,
                    willChange: "transform",
                    force3D: !0
    ...
    loading() {
        this.app.options.onceLoaded || (l.set(this.hero.titleSplit.lines, {
            overflow: "clip",
            paddingBottom: "0.1em",
            marginBottom: "-0.1em"
        }),
        this.loadingTl = l.timeline({
            ...
            paused: !0,
            onStart: () => l.set(this.main, {
                autoAlpha: 1
            })
        }),
        this.loadingTl.fromTo(this.hero.titleSplit.chars, {
            yPercent: 110
        }, {
            yPercent: 0,
            stagger: .02
        }, .8),
        this.loadingTl.fromTo(this.sections[0].querySelectorAll("img"), {
            autoAlpha: 0
        }, {
            autoAlpha: 1
        }, .8))
    ...
    resize() {
        ...
        this.init(),
        window.matchMedia("(min-width: 768px)").matches ? this.snaps ? setTimeout( () => this.snaps.resize(this.calculateSnaps()), 10) : this.snaps = new b(this.main,this.app,this.calculateSnaps()) : this.snaps && (this.snaps.destroy(),
        this.snaps = null)
    }
    getOffsetTop(e) {
        let t = 0;
