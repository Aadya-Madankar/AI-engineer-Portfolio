...
class v {
    ...
    setHeight() {
        ...
        this.list.classList.remove("fixed-height"),
        this.list.style.setProperty("--height", "auto");
        const e = this.list.offsetHeight + t.offsetHeight;
        this.list.style.setProperty("--height", e + "px"),
        this.list.classList.add("fixed-height")
    }
    init() {
        this.scrolls = [...this.items].map( (t, e) => c.create({
            trigger: t,
            start: "top 80%",
            end: "top center",
            onUpdate: r => this.quicks[e](r.progress)
        })),
    ...
    setupEmailCopy() {
        !this.mailParents.length || !this.mailElements.length || !this.copiedMessages.length || this.mailParents.forEach( (t, e) => {
            const r = this.mailElements[e]
              , i = this.copiedMessages[e];
            !t || !r || !i || (i.style.opacity = "0",
    ...
    resize() {
        this.destroyed || (this.scrolls.forEach(t => t.kill()),
        this.mailHovers.forEach(t => t?.resize()),
        this.init())
    }
    destroy() {
        this.destroyed || (this.destroyed = !0,
        this.mailHovers.forEach(t => t?.destroy()),
