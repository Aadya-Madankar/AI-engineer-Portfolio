var m = class {
    ...
    update(t) {
        this._isPlaying && (this._startTime || (this._startTime = t),
  ...
  , c = class {
    ...
    dispatch(t, e) {
        for (let i = 0; i < this.callbacks.length; i++) {
            ...
            this.callbacks[i].samples?.push(a),
    ...
    raf(t, e) {
        if (this.time += e,
        this.isRelativeFps)
            this.framesCount === 0 && this.dispatch(t, e),
            this.framesCount++,
  ...
  , f = class {
    ...
    restart() {
        ...
        for (const t of Object.values(this.framerates))
            t.framesCount = 0,
            t.time = 0,
            t.lastTickDate = performance.now();
        this.clock.reset(),
        this.play()
    }
    play() {
        !s || this.clock.isPlaying || (this.clock.play(),
        this.rafId = requestAnimationFrame(this.raf))
    }
    pause() {
        !s || !this.rafId || !this.clock.isPlaying || (cancelAnimationFrame(this.rafId),
        this.rafId = void 0,
        this.clock.pause())
    }
    get isPlaying() {
    ...
    add(t, {priority: e=0, fps: i=Number.POSITIVE_INFINITY, label: r=""}={}) {
        if (s) {
            if (typeof i == "number" || typeof i == "string" && i.endsWith("%"))
                return this.framerates[i] || (this.framerates[i] = new c(i)),
    ...
    raf = t => {
        ...
        this.fps = 1e3 / i;
        const r = performance.now();
        for (const n of Object.values(this.framerates))
            n.raf(e, i);
        const a = performance.now() - r;
        this.usage = a / i,
        this.rafId = requestAnimationFrame(this.raf)
    ...
    ;
    patch() {
        s && (window.requestAnimationFrame = (t, {priority: e=0, fps: i=Number.POSITIVE_INFINITY}={}) => t === this.raf || !t.toString().includes("requestAnimationFrame(") ? h(t) : (t.__tempusPatched || (t.__tempusPatched = !0,
    ...
    unpatch() {
        s && (window.requestAnimationFrame = h,
