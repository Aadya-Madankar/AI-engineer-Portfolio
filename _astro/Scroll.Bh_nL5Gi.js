import {T as W} from "./tempus.B_tY_6We.js";
import {g as b} from "./index.xlujcirQ.js";
import {S as L} from "./ScrollTrigger.C4gmGO9R.js";
...
var C = class {
    ...
    advance(t) {
        ...
        if (this.duration && this.easing) {
            this.currentTime += t;
            const e = H(0, this.currentTime / this.duration, 1);
            i = e >= 1;
            const s = i ? 1 : this.easing(e);
            this.value = this.from + (this.to - this.from) * s
        } else
            this.lerp ? (this.value = k(this.value, this.to, this.lerp * 60, t),
    ...
    fromTo(t, i, {lerp: e, duration: s, easing: o, onStart: l, onUpdate: n}) {
...
var Y = class {
    constructor(t, i, {autoResize: e=!0, debounce: s=250}={}) {
        this.wrapper = t,
        this.content = i,
        e && (this.debouncedResize = X(this.resize, s),
  ...
  , A = class {
    constructor(t, i={
        ...
        this.onWindowResize(),
        this.element.addEventListener("wheel", this.onWheel, S),
        this.element.addEventListener("touchstart", this.onTouchStart, S),
        this.element.addEventListener("touchmove", this.onTouchMove, S),
        this.element.addEventListener("touchend", this.onTouchEnd, S)
    }
    touchStart = {
        x: 0,
        y: 0
    ...
    destroy() {
        ...
        this.element.removeEventListener("wheel", this.onWheel, S),
        this.element.removeEventListener("touchstart", this.onTouchStart, S),
        this.element.removeEventListener("touchmove", this.onTouchMove, S),
        this.element.removeEventListener("touchend", this.onTouchEnd, S)
    }
    onTouchStart = t => {
    ...
    onWheel = t => {
        ...
        const o = s === 1 ? _ : s === 2 ? this.window.width : 1
  ...
  , R = t => Math.min(1, 1.001 - Math.pow(2, -10 * t))
  , P = class {
    _isScrolling = !1;
    _isStopped = !1;
    _isLocked = !1;
    ...
    dimensions;
    virtualScroll;
    constructor({wrapper: t=window, content: i=document.documentElement, eventsTarget: e=t, smoothWheel: s=!0, syncTouch: o=!1, syncTouchLerp: l=.075, touchInertiaExponent: n=1.7, duration: c, easing: h, lerp: p=.1, infinite: u=!1, orientation: m="vertical", gestureOrientation: r=m === "horizontal" ? "both" : "vertical", touchMultiplier: a=1, wheelMultiplier: d=1, autoResize: v=!0, prevent: f, virtualScroll: g, overscroll: y=!0, autoRaf: E=!1, anchors: N=!1, autoToggle: w=!1, allowNestedScroll: T=!1, __experimental__naiveDimensions: z=!1}={}) {
        window.lenisVersion = M,
        (!t || t === document.documentElement) && (t = window),
        typeof c == "number" && typeof h != "function" ? h = R : typeof h == "function" && typeof c != "number" && (c = 1),
        this.options = {
            wrapper: t,
            content: i,
            eventsTarget: e,
    ...
    onTransitionEnd = t => {
        if (t.propertyName.includes("overflow")) {
            const i = this.isHorizontal ? "overflow-x" : "overflow-y"
              , e = getComputedStyle(this.rootElement)[i];
    ...
    onClick = t => {
        const e = t.composedPath().find(s => s instanceof HTMLAnchorElement && (s.getAttribute("href")?.startsWith("#") || s.getAttribute("href")?.startsWith("/#") || s.getAttribute("href")?.startsWith("./#")));
        if (e) {
            const s = e.getAttribute("href");
            if (s) {
                const o = typeof this.options.anchors == "object" && this.options.anchors ? this.options.anchors : void 0;
                let l = `#${s.split("#")[1]}`;
                ["#", "/#", "./#", "#top", "/#top", "./#top"].includes(s) && (l = 0),
                this.scrollTo(l, o)
            }
        }
    }
    ...
    onVirtualScroll = t => {
        ...
        const n = i === 0 && e === 0;
        if (this.options.syncTouch && o && s.type === "touchstart" && n && !this.isStopped && !this.isLocked) {
            this.reset();
            return
        }
        const h = this.options.gestureOrientation === "vertical" && e === 0 || this.options.gestureOrientation === "horizontal" && i === 0;
        if (n || h)
            return;
        let p = s.composedPath();
        p = p.slice(0, p.indexOf(this.rootElement));
        const u = this.options.prevent;
        if (p.find(f => f instanceof HTMLElement && (typeof u == "function" && u?.(f) || f.hasAttribute?.("data-lenis-prevent") || o && f.hasAttribute?.("data-lenis-prevent-touch") || l && f.hasAttribute?.("data-lenis-prevent-wheel") || this.options.allowNestedScroll && this.checkNestedScroll(f, {
            deltaX: i,
            deltaY: e
        }))))
            return;
        ...
        let r = e;
        this.options.gestureOrientation === "both" ? r = Math.abs(e) > Math.abs(i) ? e : i : this.options.gestureOrientation === "horizontal" && (r = i),
        (!this.options.overscroll || this.options.infinite || this.options.wrapper !== window && this.limit > 0 && (this.animatedScroll > 0 && this.animatedScroll < this.limit || this.animatedScroll === 0 && e > 0 || this.animatedScroll === this.limit && e < 0)) && (s.lenisStopPropagation = !0),
        s.cancelable && s.preventDefault();
        const a = o && this.options.syncTouch
          , v = o && s.type === "touchend";
        v && (r = Math.sign(this.velocity) * Math.pow(Math.abs(this.velocity), this.options.touchInertiaExponent)),
        this.scrollTo(this.targetScroll + r, {
            programmatic: !1,
            ...a ? {
    ...
    ;
    scrollTo(t, {offset: i=0, immediate: e=!1, lock: s=!1, duration: o=this.options.duration, easing: l=this.options.easing, lerp: n=this.options.lerp, onStart: c, onComplete: h, force: p=!1, programmatic: u=!0, userData: m}={}) {
        if (!((this.isStopped || this.isLocked) && !p)) {
            if (typeof t == "string" && ["top", "left", "start"].includes(t))
                ...
                let r;
                if (typeof t == "string" ? r = document.querySelector(t) : t instanceof HTMLElement && t?.nodeType && (r = t),
                r) {
                    if (this.options.wrapper !== window) {
                        const d = this.rootElement.getBoundingClientRect();
                        i -= this.isHorizontal ? d.left : d.top
                    }
            ...
            if (typeof t == "number") {
                ...
                this.options.infinite) {
                    if (u) {
                        ...
                        const r = t - this.animatedScroll;
                        r > this.limit / 2 ? t = t - this.limit : r < -this.limit / 2 && (t = t + this.limit)
                    }
                } else
                    t = H(0, t, this.limit);
                ...
                typeof o == "number" && typeof l != "function" ? l = R : typeof l == "function" && typeof o != "number" && (o = 1),
    ...
    checkNestedScroll(t, {deltaX: i, deltaY: e}) {
        ...
        if (s - (o.time ?? 0) > 2e3) {
            ...
            const T = w.overflowX
              , z = w.overflowY;
            if (l = ["auto", "overlay", "scroll"].includes(T),
            n = ["auto", "overlay", "scroll"].includes(z),
            o.hasOverflowX = l,
            o.hasOverflowY = n,
            !l && !n || a === "vertical" && !n || a === "horizontal" && !l)
                return !1;
            p = t.scrollWidth,
            u = t.scrollHeight,
            m = t.clientWidth,
            ...
            m = o.clientWidth,
            r = o.clientHeight;
        if (!l && !n || !c && !h || a === "vertical" && (!n || !h) || a === "horizontal" && (!l || !c))
            return !1;
        let d;
        if (a === "horizontal")
            d = "x";
...
class F {
    constructor() {
        this.init(),
        L.addEventListener("refresh", () => this.lenis.resize()),
        this.lenis.on("scroll", L.update),
        W.add(i => this.lenis.raf(i)),
        b.ticker.remove(b.updateRoot),
        W.add(i => b.updateRoot(i / 1e3))
    }
    init() {
        this.lenis = new P({
            duration: window.innerWidth > 768 ? .6 : 1,
            easing: i => i === 1 ? 1 : 1 - Math.pow(2, -10 * i),
            direction: "vertical",
