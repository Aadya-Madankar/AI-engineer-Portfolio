...
class d {
    ...
    init() {
        ...
        this.data.current.namespace === "home" && (this.app.gl.world.models.forEach(t => {
            this.tl.to(t.inner.material.uniforms.uOpacity, {
                value: 0,
                duration: .8,
                ease: "power2.out"
            }, .2),
            this.tl.to(t.material.material.uniforms.uOpacity, {
                value: 0,
                duration: .8,
                ease: "power2.out"
            }, "<")
        ...
        this.tl.to(this.app.gl.world.footerGradient.material.uniforms.uProgress, {
            value: 0,
            duration: .8,
            overwrite: !0
        }, "<")),
        this.app.gl.world.footerModel && this.tl.to(this.app.gl.world.footerModel.material.material.uniforms.uOpacity, {
            value: 0,
            duration: .8,
            ease: "power2.out"
        }, .2)
    ...
    async start() {
        ...
        this.data.next.namespace === "home" ? (this.app.page.loader.tl.progress(1),
        document.body.classList.remove("bg-black", "text-white"),
        document.body.classList.add("bg-grey", "text-black"),
        setTimeout( () => document.body.classList.remove("white-nav", "high-noise"), 500)) : setTimeout( () => document.body.classList.add("white-nav", "high-noise"), 240)
    }
    async complete() {
        if (e.set(this.nextContainer, {
        ...
        this.data.next.namespace === "home") {
            const t = window.innerWidth < 768 ? .1 : 0;
            await this.app.page.loader.loadModel(t),
            document.querySelector(".canvas-container").style.zIndex = -1
        } else
            this.data.next.namespace === "about" && (document.querySelector(".canvas-container").style.zIndex = 10)
    }
}
export {d as default};
